...
resource "yandex_compute_instance" "bastion" {
  ...
  hostname    = "${var.resource_prefix}-bastion"  # Формирует FQDN
  ...
  resources {
    cores         = 2
    memory        = 2
    core_fraction = 20  # Базовый уровень 20%
  }
  ...
  boot_disk {
    initialize_params {
      ...
      type     = "network-ssd"  # SSD для скорости
      size     = 10  # GB
    }
  }
