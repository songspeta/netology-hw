...
## 7. Disk Inodes (Свободные inode)

**Причина:** платформа сохраняет текстовые отчеты на диск, то есть постоянно создает новые файлы. Каждый файл занимает один inode, а их количество в файловой системе ограничено. При большом количестве мелких отчетов inode могут закончиться РАНЬШЕ, чем свободное место на диске — в этом случае система вернет ошибку `No space left on device` и не сможет сохранить новый отчет, даже если `df -h` показывает свободные гигабайты.

*  **Что мониторить:** процент свободных inode (`df -i`; в node_exporter — `node_filesystem_files_free / node_filesystem_files`; в telegraf — поля `inodes_total`, `inodes_free` плагина `inputs.disk`).
...
## Ответ на вопрос преподавателя: что такое inodes и нужно ли их отслеживать

**inode (index node)** — это структура данных в файловой системе (ext4, xfs и т.д.), которая описывает файл: метаданные (владелец, права, время создания) и указатели на блоки с данными на диске. Каждый файл, директория или симлинк занимает ровно один inode, а общее их количество ограничено.

**Нужно ли отслеживать inodes в данном случае — да, нужно.** По условию задачи платформа пишет на диск текстовые отчеты, то есть постоянно создает файлы. Если отчетов много и они мелкие (или не очищаются временные файлы), таблица inode может заполниться, и сервис перестанет сохранять новые отчеты с ошибкой `No space left on device` при формально свободном месте на диске. Это прямая деградация сервиса и нарушение SLA.

**Но менеджеру продукта inodes в сыром виде показывать не нужно** — это техническая метрика инфраструктуры. Для DevOps-команды мы мониторим inodes на уровне алертов (например, "свободных inode < 10%"), а для менеджера их состояние выражается через понятные бизнес-метрики: "Доступность сервиса", "Успешность сохранения отчетов", "Выполнение SLA". То есть inodes — это первопричина, а бизнес-метрики — то, как мы показываем её влияние менеджеру.
