...
# Подсеть для базы данных (zone B)
resource "yandex_vpc_subnet" "develop_db" {
  name           = "develop-fops-${var.flow}-ru-central1-b-db"
  zone           = "ru-central1-b"
  network_id     = yandex_vpc_network.develop.id
...
resource "yandex_vpc_security_group" "db_sg" {
  ...
  # Разрешить подключения только от веб-серверов
  ingress {
    description    = "Allow PostgreSQL from webservers"
    protocol       = "TCP"
    port           = 5432
    v4_cidr_blocks = ["10.0.1.0/24", "10.0.2.0/24"]  # подсети web-a и web-b
  }
